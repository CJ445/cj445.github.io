...
const Taskbar = () => {
  ...
  return (
    <>
      <nav aria-label="Primary" className="fixed top-4 left-4 right-4 z-50 bg-custom-purple border-4 border-black rounded-full px-4 py-3 shadow-neo flex justify-between items-center max-w-7xl mx-auto">

        <div className="text-xl md:text-2xl font-shrikhand text-black ml-2 mr-3 shrink-0">
          PORTFOLIO
        </div>
        ...
        <div className="hidden xl:flex items-center gap-2 bg-custom-yellow text-black px-4 py-1 rounded-full font-mono text-sm border-black border-2 border-b-4 border-r-4">
          <span>█</span>
          <span>{formatDate(currentTime)}</span>
        </div>

        <button
          className="lg:hidden text-white text-2xl bg-black w-11 h-11 flex items-center justify-center rounded-full border-2 border-white"
      ...
      {isMenuOpen && (
        <div className="fixed top-24 left-4 right-4 z-40 bg-custom-purple border-4 border-black rounded-3xl p-4 flex flex-col gap-3 shadow-neo animate-bounce-in lg:hidden max-w-7xl mx-auto">
          ...
          <div className="flex xl:hidden items-center gap-2 bg-custom-yellow text-black px-4 py-3 rounded-xl font-mono text-sm border-black border-2 border-b-4 border-r-4 justify-center">
            <span>█</span>
            <span>{formatDate(currentTime)}</span>
          </div>
        </div>
